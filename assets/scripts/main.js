...
async function getRecipes() {
    ...
    let recipesFromLocalStorage = localStorage.getItem("recipes");

    if (recipesFromLocalStorage !== null) {
          const parsedRecipes = JSON.parse(recipesFromLocalStorage);
    ...
    // The 'async' keyword here is crucial because we'll use 'await' inside.
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            //            resolve() method.
            // We know we've finished retrieving all recipes because Promise.all has resolved.
            saveRecipesToStorage(recipes);
            resolve(recipes);

        } catch (error) {
            // A10. TODO - Log any errors from catch using console.error
            console.error("Error fetching recipes from network:", error);
